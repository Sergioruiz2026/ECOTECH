...
def enviar_codigo_verificacion(destinatario: str, codigo: str):
    ...
    configuracion_faltante = [
        nombre
        for nombre, valor in (
            ("ECOTECH_SMTP_HOST", host),
            ("ECOTECH_SMTP_USER", usuario),
            ("ECOTECH_SMTP_PASSWORD", contraseña),
        )
        if not valor
    ]
    if configuracion_faltante:
        raise RuntimeError(
            "Falta configurar: " + ", ".join(configuracion_faltante) + ". "
            "ECOTECH_SMTP_FROM es opcional y usa ECOTECH_SMTP_USER por defecto."
        )
